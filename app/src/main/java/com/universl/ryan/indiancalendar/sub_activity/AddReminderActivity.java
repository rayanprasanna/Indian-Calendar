...
import com.universl.ryan.indiancalendar.data_reminder.AlarmScheduler;
...
public class AddReminderActivity extends AppCompatActivity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Initialize Views
        Toolbar mToolbar = findViewById(R.id.search_bar);
        ...
        Objects.requireNonNull(getSupportActionBar()).setTitle(Html.fromHtml("<font color='#ffffff'>अनुस्मारक जोड़ें</font>"));
...
*/
        ...
        if (mActive.equals("true")) {
            if (mRepeat.equals("true")) {
                ...
                System.err.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>  Fuck Bull Shit : <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" + mCurrentReminderUri.getPath());
            }
